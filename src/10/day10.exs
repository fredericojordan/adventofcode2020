...
defmodule Day10 do
  ...
  defp read_input_file() do
    {:ok, adaptors_file} = File.read("input.txt")
  ...
  def test_input_1(),
    do: [16, 10, 15, 5, 1, 11, 7, 19, 6, 12, 4]

  def test_input_2(),
    do: [28, 33, 18, 42, 31, 14,46, 20, 48, 47, 24,23, 49, 45, 19, 38,39, 11, 1, 32, 25,35, 8, 17, 7, 9,4, 2, 34, 10, 3]


  def part_1() do
    read_input_file()
    |> Enum.sort()
    |> (fn list -> [0] ++ list ++ [Enum.max(list) + 3] end).()
    |> Enum.chunk_every(2, 1, :discard)
    |> Enum.map(fn [a, b] -> b-a end)
    |> (fn list -> Enum.count(list, & &1 == 1) * Enum.count(list, & &1 == 3) end).()
  end

  def tribonacci(0), do: 1
  def tribonacci(1), do: 1
  def tribonacci(2), do: 2
  ...
  def part_2() do
    ...
    |> Enum.map(fn [a, b] -> b-a end)
    |> Enum.chunk_by(& &1 == 1)
    |> Enum.map(fn x -> Enum.count(x, & &1 == 1) end)
