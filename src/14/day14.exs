...
defmodule Day14 do
  ...
  defp read_input_file() do
    {:ok, program_file} = File.read("input.txt")
  ...
  defp run_value_masking_program("mask = " <> new_mask, {memory_map, _mask}),
    do: {memory_map, new_mask}

  defp run_value_masking_program("mem" <> memory_entry, {memory_map, mask}),
    do: {put_masked_value(memory_map, mask, memory_entry), mask}

  defp parse_memory_entry(memory_entry) do
    Regex.scan(~r/\[([\d]+)\] = ([\d]+)/, memory_entry, capture: :all_but_first)
    |> List.first()
    |> Enum.map(&String.to_integer/1)
  end

  defp put_masked_value(memory_map, mask, memory_entry) do
    [position, value] = parse_memory_entry(memory_entry)

    Map.put(memory_map, position, mask_number(value, mask))
  end

  defp mask_digit({_digit, "0"}), do: 0
  defp mask_digit({_digit, "1"}), do: 1
  defp mask_digit({digit, _}), do: digit

  defp pad_leading_zeros(list, total_length),
    do: List.duplicate(0, total_length - Enum.count(list)) ++ list

  defp mask_number(number, mask) do
    number
    |> Integer.digits(2)
    |> pad_leading_zeros(36)
    |> Enum.zip(String.graphemes(mask))
    |> Enum.map(&mask_digit/1)
    |> Integer.undigits(2)
  end

  def part_1() do
    read_input_file()
    |> Enum.reduce({%{}, ""}, &run_value_masking_program/2)
    |> elem(0)
    |> Map.values()
    |> Enum.sum()
  end

  defp put_masked_memory(memory_map, mask, memory_entry) do
    [position, value] = parse_memory_entry(memory_entry)

    position
    |> generate_masked_positions(mask)
    |> Enum.reduce(memory_map, fn pos, map -> Map.put(map, pos, value) end)
  end

  defp mask_memory_digit({digit, "0"}), do: Integer.to_string(digit)
  defp mask_memory_digit({_digit, "1"}), do: "1"
  defp mask_memory_digit({_digit, "X"}), do: "X"

  defp generate_masked_positions(position, mask) do
    position
    |> Integer.digits(2)
    |> pad_leading_zeros(36)
    |> Enum.zip(String.graphemes(mask))
    |> Enum.map(&mask_memory_digit/1)
    |> Enum.reduce(&Kernel.<>/2)
    |> generate_masked_positions()
  end

  defp generate_masked_positions(masked_position) do
    case Regex.run(~r/X/, masked_position) do
      nil ->
        [String.to_integer(masked_position, 2)]

      _ ->
        generate_masked_positions(String.replace(masked_position, "X", "1", global: false)) ++
          generate_masked_positions(String.replace(masked_position, "X", "0", global: false))
    end
  end

  defp run_memory_masking_program("mask = " <> new_mask, {memory_map, _mask}),
    do: {memory_map, new_mask}

  defp run_memory_masking_program("mem" <> memory_entry, {memory_map, mask}),
    do: {put_masked_memory(memory_map, mask, memory_entry), mask}

  def part_2() do
    read_input_file()
    |> Enum.reduce({%{}, ""}, &run_memory_masking_program/2)
    |> elem(0)
    |> Map.values()
    |> Enum.sum()
  end
...
IO.inspect(Day14.part_1())
IO.inspect(Day14.part_2())
