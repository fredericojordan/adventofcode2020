...
defmodule Day05 do
  ...
  defp read_input_file() do
    {:ok, boarding_passes_file} = File.read("input.txt")
  ...
  defp get_row(boarding_pass), do: get_row(String.slice(boarding_pass, 0..6), 0, 128)
  defp get_row("F", min, _max), do: min
  defp get_row("B", _min, max), do: max
  defp get_row("F" <> rest, min, max), do: get_row(rest, min, div(max+min+1, 2)-1)
  defp get_row("B" <> rest, min, max), do: get_row(rest, div(max+min+1, 2), max)

  defp get_column(boarding_pass), do: get_column(String.slice(boarding_pass, 7..9), 0, 7)
  defp get_column("L", min, _max), do: min
  defp get_column("R", _min, max), do: max
  defp get_column("L" <> rest, min, max), do: get_column(rest, min, div(max+min+1, 2)-1)
  defp get_column("R" <> rest, min, max), do: get_column(rest, div(max+min+1, 2), max)

  defp get_seat_id(boarding_pass), do: 8 * get_row(boarding_pass) + get_column(boarding_pass)
  ...
  def part_2() do
    ...
    1..Enum.max(seat_ids)
    |> Stream.filter(& &1 not in seat_ids)
    |> Enum.reduce_while(0, fn x, acc -> if x == acc + 1, do: {:cont, x}, else: {:halt, x} end)
  end

end

IO.inspect(Day05.part_1())
IO.inspect(Day05.part_2())
