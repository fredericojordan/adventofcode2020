...
defmodule Day15 do
  ...
  For example, suppose the starting numbers are 0,3,6:
      ...
      - Turn 5: Next, again consider the last number spoken, 0. Since it had been spoken before, the next number to
        ...
        number of the time it was most recently spoken before then (turn 1). Thus, the 5th number spoken is 4 - 1, 3.
  ...
  defp input(), do: [13, 0, 10, 12, 1, 5, 8]
  ...
  def part_1() do
    ...
    input
    |> Enum.reverse()
    |> Stream.iterate(&play_saying_game/1)
    |> Enum.at(2020 - Enum.count(input))
    |> List.first()
  end

  defp play_saying_game([latest_num | rest]) do
    case Enum.find_index(rest, &(&1 == latest_num)) do
      nil -> [0 | [latest_num | rest]]
      index -> [index + 1 | [latest_num | rest]]
    end
  end
  ...
  def part_2() do
    ...
    |> Enum.reverse()
    |> Stream.iterate(&play_saying_game/1)
    |> Enum.at(30_000_000 - Enum.count(input))
    |> List.first()
  end
end

IO.inspect(Day15.part_1())
IO.inspect(Day15.part_2())
