...
defmodule Day09 do
  ...
  defp read_input_file() do
    {:ok, xmas_data_file} = File.read("input.txt")
  ...
  def test_input(),
    do: [35, 20, 15, 25, 47, 40, 62, 55, 65, 95, 102, 117, 150, 182, 127, 219, 299, 277, 309, 576]

  defp all_pairs(list) do
    for x <- list, y <- list, x != y, do: [x, y]
  end

  def is_sum_of_previous(list) do
    [target | parts] = Enum.reverse(list)
  ...
  def find_invalid_number(list, preamble_length) do
    list
    |> Stream.chunk_every(preamble_length+1, 1)
    |> Stream.reject(&is_sum_of_previous/1)
    |> Enum.take(1)
    |> List.first()
    |> List.last()
  end
  ...
  defp contiguous_sets(list) do
    for chunk_size <- 2..length(list) do
      Enum.chunk_every(list, chunk_size, 1, :discard)
    end
  end

  defp find_weakness(list, preamble_length) do
    target = find_invalid_number(list, preamble_length)

    list
    |> contiguous_sets()
    |> Enum.reduce(&Kernel.++/2)
    |> Enum.filter(fn set -> Enum.reduce(set, &Kernel.+/2) == target end)
    |> Enum.take(1)
    |> List.first()
    |> (fn set -> Enum.min(set) + Enum.max(set) end).()
  end
...
IO.inspect(Day09.part_1())
IO.inspect(Day09.part_2())
