...
defmodule Day06 do
  ...
  defp read_input_file() do
    ...
    |> Stream.map(fn ans -> Enum.filter(ans, &(&1 != "")) end)
  end

  defp count_any_answers(answer_list) do
    answer_list
  ...
  defp count_all_answers(answer_list) do
    [first | rest] =
      answer_list
      |> Enum.map(&String.graphemes/1)
    ...
    case rest do
      [] -> Enum.count(first)
      _ -> Enum.count(first, fn x -> Enum.all?(rest, & Enum.member?(&1, x)) end)
    end
  end

  def part_1() do
    read_input_file()
    |> Stream.map(&count_any_answers/1)
    |> Enum.sum()
  end

  def part_2() do
    read_input_file()
    |> Stream.map(&count_all_answers/1)
    |> Enum.sum()
  end

end

IO.inspect(Day06.part_1())
IO.inspect(Day06.part_2())
